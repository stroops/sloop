...
A hook installs at one of two levels, and the design must make this explicit:
...
context to stdout, and the provider injects that into the session as additional context. Sloop never
...
- **Dynamic, not static.** The hook runs each session, so it can surface what was learned/saved since
  last time, which the static `AGENTS.md` can't.
